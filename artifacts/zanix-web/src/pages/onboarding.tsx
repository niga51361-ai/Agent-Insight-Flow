...
import {
  ArrowRight, ArrowLeft, CheckCircle2, Sparkles, AlertCircle,
...
// ── Constants ─────────────────────────────────────────────────────
const INTERESTS = [
  { id: "Technology",   emoji: "💻", color: "from-cyan-500/20 to-blue-500/20",   border: "border-cyan-500/30",   active: "bg-cyan-500/15 border-cyan-500/50 text-cyan-300" },
  { id: "Business",     emoji: "📊", color: "from-amber-500/20 to-orange-500/20", border: "border-amber-500/30",  active: "bg-amber-500/15 border-amber-500/50 text-amber-300" },
  { id: "Science",      emoji: "🔬", color: "from-violet-500/20 to-indigo-500/20",border: "border-violet-500/30", active: "bg-violet-500/15 border-violet-500/50 text-violet-300" },
  { id: "Arts",         emoji: "🎨", color: "from-pink-500/20 to-rose-500/20",    border: "border-pink-500/30",   active: "bg-pink-500/15 border-pink-500/50 text-pink-300" },
  { id: "Sports",       emoji: "⚽", color: "from-emerald-500/20 to-teal-500/20", border: "border-emerald-500/30",active: "bg-emerald-500/15 border-emerald-500/50 text-emerald-300" },
  { id: "Travel",       emoji: "✈️", color: "from-sky-500/20 to-cyan-500/20",     border: "border-sky-500/30",    active: "bg-sky-500/15 border-sky-500/50 text-sky-300" },
  { id: "Food",         emoji: "🍜", color: "from-orange-500/20 to-red-500/20",   border: "border-orange-500/30", active: "bg-orange-500/15 border-orange-500/50 text-orange-300" },
  { id: "Music",        emoji: "🎵", color: "from-purple-500/20 to-violet-500/20",border: "border-purple-500/30", active: "bg-purple-500/15 border-purple-500/50 text-purple-300" },
  { id: "Learning",     emoji: "📚", color: "from-lime-500/20 to-green-500/20",   border: "border-lime-500/30",   active: "bg-lime-500/15 border-lime-500/50 text-lime-300" },
  { id: "Finance",      emoji: "💰", color: "from-yellow-500/20 to-amber-500/20", border: "border-yellow-500/30", active: "bg-yellow-500/15 border-yellow-500/50 text-yellow-300" },
  { id: "Health",       emoji: "🧘", color: "from-teal-500/20 to-emerald-500/20", border: "border-teal-500/30",   active: "bg-teal-500/15 border-teal-500/50 text-teal-300" },
  { id: "Design",       emoji: "✏️", color: "from-rose-500/20 to-pink-500/20",    border: "border-rose-500/30",   active: "bg-rose-500/15 border-rose-500/50 text-rose-300" },
];
...
const PERSONALITIES = [
  {
    ...
    icon: Brain,
    name: "Analytical",
    desc: "Data-driven, logical, precise. Perfect for research and problem-solving.",
    emoji: "🔬",
  ...
  },
  {
    ...
    icon: Sparkles,
    name: "Creative",
    desc: "Imaginative, innovative, artistic. Ideal for brainstorming and design.",
    emoji: "🎨",
  ...
  },
  {
    ...
    icon: Smile,
    name: "Friendly",
    desc: "Warm, approachable, empathetic. Great for everyday tasks and support.",
    emoji: "😊",
  ...
  },
  {
    ...
    icon: Briefcase,
    name: "Professional",
    desc: "Formal, efficient, expert. Best for business and executive workflows.",
    emoji: "💼",
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/me", { signal: AbortSignal.timeout(6000) })
      .then(async r => {
        if (!r.ok) { setAuthError(true); setTimeout(() => setLocation("/auth"), 2000); return; }
        const { user } = await r.json();
        setUserName(user.name?.split(" ")[0] || "");
        if (user.onboardingCompleted) { setLocation("/chat"); return; }
      })
      .catch(() => { setAuthError(true); setTimeout(() => setLocation("/auth"), 2000); })
  ...
  const handleNext = () => {
    if (step === 1) {
      if (!data.agentName.trim()) { setNameError("Please give your agent a name"); return; }
      if (data.agentName.length < 2) { setNameError("Name must be at least 2 characters"); return; }
      setNameError("");
    }
  ...
  // ── Auth error ─────────────────────────────────────────────────
  if (authError) return (
    <div className="min-h-screen bg-[hsl(228_22%_4%)] flex items-center justify-center px-5">
      <motion.div
      ...
      >
        ...
        <p className="text-white/45 text-sm">Please sign in to continue.</p>
  ...
  if (initializing) return (
    <div className="min-h-screen bg-[hsl(228_22%_4%)] flex flex-col">
  ...
  // ── Step renders ───────────────────────────────────────────────
  const agentDisplayName = data.agentName || "your agent";
  ...
  return (
    <div className="min-h-screen bg-[hsl(228_22%_4%)] flex flex-col">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_40%_30%_at_80%_90%,hsl(186_100%_50%/0.05),transparent)]" />
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="text-center space-y-5">
                  ...
                  >
                    ...
                    <span className="absolute -top-1 -right-1 w-7 h-7 rounded-full bg-emerald-500 border-2 border-[hsl(228_22%_4%)] flex items-center justify-center shadow-[0_0_12px_hsl(160_84%_39%/0.5)]">
                  ...
                  <div>
                    ...
                    <p className="text-white/40 text-base">
                      Let&apos;s set up your personal AI agent in just a few steps.
                    </p>
                  </div>
                </div>
                ...
                <div className="glass-elevated rounded-3xl p-7 border border-white/7 space-y-5">
                  <div>
                    ...
                    <div className={cn(
                      ...
                      <Bot className="absolute left-4 w-5 h-5 text-white/20 pointer-events-none" />
                      <input
                        type="text"
                        placeholder="e.g. Alex, Nova, Nexus…"
                        value={data.agentName}
                        onChange={e => { setData(p => ({ ...p, agentName: e.target.value })); setNameError(""); }}
                        onKeyDown={e => e.key === "Enter" && handleNext()}
                        maxLength={50}
                        autoFocus
                        className="w-full bg-transparent py-4 pl-12 pr-16 text-base text-white placeholder:text-white/20 focus:outline-none"
                      />
                      <span className="absolute right-4 text-xs text-white/20">{data.agentName.length}/50</span>
                    </div>
                    {nameError && (
                    ...
                    <p className="text-xs text-white/25 mt-2 pl-1">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-white/80 mb-3 block">
                      Brief background about you <span className="text-white/30 font-normal">(optional)</span>
                    </label>
                    <textarea
                      placeholder="I'm a software engineer focused on AI products. I love problem-solving and building at scale…"
                    ...
                    />
                    <p className="text-xs text-white/20 mt-1.5 pl-1 text-right">{data.userBackground.length}/500</p>
                  </div>
            ...
            {step === 2 && (
              ...
              >
                <div className="text-center space-y-2">
                  ...
                  <p className="text-white/40 text-base">
                    {agentDisplayName} will use this to personalise your experience.
                  </p>
                </div>
                ...
                {data.userInterests.length > 0 && (
                  ...
                  >
                    {data.userInterests.length} selected · {data.userInterests.join(", ")}
                  </motion.p>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {data.userInterests.length === 0 ? "Skip for now" : "Continue"}
            ...
            {step === 3 && (
              ...
              >
                <div className="text-center space-y-2">
                  <h2 className="text-3xl sm:text-4xl font-display font-black text-white leading-tight">
                    Pick {agentDisplayName}&apos;s style
                  </h2>
                  <p className="text-white/40 text-base">
                    How should your agent communicate with you?
                  </p>
                </div>
                  ...
                  {PERSONALITIES.map(p => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={cn(
                          "relative p-5 rounded-2xl border-2 text-left transition-all duration-250 overflow-hidden group",
                          isSelected
                  ...
                  >
                    ...
                    {loading ? (
                      <>
                        ...
                        <span>Launching {agentDisplayName}…</span>
                      </>
                    ) : (
                      <>
                        <Rocket className="w-4 h-4" />
                        <span>Launch {agentDisplayName}</span>
                      </>
                    )}
                  </motion.button>
                </div>
